...
from kepler.io.importer import ImporterError, import_json_string
from kepler.types import *

# TODO - adapt shift tests

# Test for successful parsing

def test_success() -> None:
    problem = import_json_string('''
        {
            "courses": [
                {
                    "id": "C1",
...
def test_root_not_object() -> None:
    with pytest.raises(ImporterError) as einfo:
        import_json_string('[]')
...
def test_course_year_is_not_positive() -> None:
    with pytest.raises(ImporterError):
        import_json_string('''
            {
                "courses": [
...
def test_shifts_not_array() -> None:
    with pytest.raises(ImporterError):
        import_json_string('''
            {
                "courses": [
