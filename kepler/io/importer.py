...
def import_json_file(path: str) -> SchedulingProblem: # pragma: no coverage
    try:
        with open(path, mode='r', encoding='utf-8') as f:
            root_json = json.load(f)
    except IOError as e:
        raise ImporterError(f'Failed to read JSON file {path}: {e}') from e
    except json.JSONDecodeError as e:
        raise ImporterError(f'Failed to parse JSON file {path}: {e}') from e

    return __parse_json_root(root_json)

def import_json_string(json_string: str) -> SchedulingProblem:
    try:
        root_json = json.loads(json_string)
    except json.JSONDecodeError as e:
        raise ImporterError(f'Failed to parse JSON string: {e}') from e

    return __parse_json_root(root_json)

def __parse_json_root(root_json: object) -> SchedulingProblem:
    __assert_dict_with_keys(root_json, {'students', 'courses'}, 'the JSON\'s root')
    root_json = cast(dict[str, object], root_json)
    ...
    try:
        ...
        raise ImporterError(f'Invalid scheduling problem: {e}') from e

def __parse_courses(courses_json: object) -> list[Course]:
    __assert_type(courses_json, list, 'courses')
    courses_json = cast(list[object], courses_json)
...
def __parse_course(course_json: object) -> Course:
    ...
    course_json = cast(dict[str, object], course_json)
...
def __parse_shift(shift_json: object) -> Shift:
    ...
    shift_json = cast(dict[str, object], shift_json)
...
def __parse_timeslot(timeslot_json: object) -> Timeslot:
    ...
    timeslot_json = cast(dict[str, object], timeslot_json)
...
def __parse_student(student_json: object, courses: dict[str, Course]) -> Student:
    __assert_dict_with_keys(student_json, {'number', 'enrollments'}, 'student')
    student_json = cast(dict[str, object], student_json)
    ...
    try:
        return Student(number, student_courses, schedule)
    except StudentError as e:
        raise ImporterError(f'Invalid student {number}: {e}') from e
...
def __parse_enrollements(enrollments_json: object, courses: dict[str, Course]) -> list[Course]:
    ...
    enrollments_json = cast(list[object], enrollments_json)
...
def __parse_enrollment(enrollment_json: object, courses: dict[str, Course]) -> Course:
    ...
    enrollment_json = cast(str, enrollment_json)

    course = courses.get(enrollment_json)
    if course is None:
        raise ImporterError(f'Course {enrollment_json} in enrollment was not found')
...
def __parse_schedule(
    ...
    schedule_json = cast(list[object], schedule_json)
    ...
    try:
        ...
        raise ImporterError(f'{student_number}\'s schedule is invalid: {e}') from e
...
def __assert_dict_with_keys(
    ...
    value_json = cast(dict[str, object], value_json)
